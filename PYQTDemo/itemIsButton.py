...
class MyButtonDelegate(QItemDelegate):
    ...
    def paint(self, painter, option, index):
        if not self.parent().indexWidget(index):
            button_read = QPushButton(self.tr('读'),self.parent(),clicked=self.parent().cellButtonClicked)
            button_write = QPushButton(self.tr('写'),self.parent(),clicked=self.parent().cellButtonClicked)
